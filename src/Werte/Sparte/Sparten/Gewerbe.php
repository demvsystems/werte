<?php

namespace Demv\Werte\Sparte\Sparten;

/**
 * Interface Gewerbe
 * @package Demv\Werte\Sparte\Sparten
 */
interface Gewerbe
{
    const BAUHERREN_HAFTPFLICHT               = 320;
    const BAULEISTUNGSVERSICHERUNG            = 353;
    const BERUFSHAFTPFLICHT                   = 173;
    const BETREIBERHAFTPFLICHT                = 493;
    const BETRIEBSHAFTPFLICHT                 = 174;
    const BETRIEBSSCHLIESSUNGSVERSICHERUNG    = 338;
    const BETRIEBSUNTERBRECHUNG               = 260;
    const BUERGSCHAFT                         = 305;
    const CYBER                               = 401;
    const D_O                                 = 240;
    const ELEKTRONIK                          = 263;
    const FEUERVERSICHERUNG                   = 377;
    const FLOTTENVERSICHERUNG                 = 180;
    const FRACHTFUEHRER                       = 379;
    const GEBAEUDE                            = 177;
    const GESCHAEFTSINHALT                    = 176;
    const KFZ                                 = 179;
    const KFZ_ANHAENGER                       = 559;
    const KFZ_SCHLEPPER_ZUGMASCHINE           = 423;
    const KFZ_LANDWIRTSCHAFTLICHE_ZUGMASCHINE = 593;
    const KFZ_LIEFERWAGEN                     = 531;
    const KFZ_LKW                             = 406;
    const KFZ_MOTORRAD                        = 412;
    const KFZ_QUAD                            = 433;
    const KFZ_TRIKE                           = 436;
    const GLAS                                = 232;
    const GRUPPEN_UNFALLVERSICHERUNG          = 234;
    const LANDWIRTSCHAFT                      = 382;
    const LUFTFAHRTKASKOVERSICHERUNG          = 402;
    const MASCHINEN                           = 378;
    const MONTAGEVERSICHERUNG                 = 386;
    const PHOTOVOLTAIKVERSICHERUNG            = 256;
    const RECHTSSCHUTZ                        = 233;
    const SACHVERSICHERUNG                    = 172;
    const TRANSPORT                           = 178;
    const UMWELTSCHADENVERSICHERUNG           = 317;
    const VERANSTALTERHAFTPFLICHT             = 259;
    const VERMOEGENSCHADENHAFTPFLICHT         = 175;
    const WASSERSPORTKASKO                    = 459;
}
